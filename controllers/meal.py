...
class Meal(Resource):
    ...
    parser.add_argument('name', type=str, required=True, help='Należy podać nazwę!')
    ...
    @jwt_required()  
    def get(self, id):
        meal = MealModel.find_by_id(id)
        if meal:
            meal_json = meal.json()
            self.logger.info(f'Posiłek: {meal_json}')
            return meal_json
    ...
    def delete(self, id):
        ...
        if meal:
            meal.delete_from_db()
            return {'message': 'Posiłek został usunięty z bazy'}, 200
        return {'message': 'Nie ma takiego posiłku w bazie danych'}, 404
    ...
    def patch(self, id):
        ...
        meal = MealModel.find_by_id(id)
        if meal is None:
            meal = MealModel(data['name'], data['price'], data['potato_quantity'], data['meal_type'], data['diary_addons'], data['protein_addons'])
        else:
            meal.name = data['name']
...
class MealList(Resource):
    ...
    def post(self):
        ...
        if MealModel.find_by_name(data['name']):
            return {'message': "Posiłek z nazwą: '{}' już istnieje w bazie danych.".format(data['name'])}, 400
